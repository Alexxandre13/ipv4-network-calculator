...
export const checkRangeAndType = (name: string, value: number, min: number, max: number): void => {
...
export const checkIp = (ip: iIPv4): void[] => {
  return Object.values(ip).map((octet: number) => checkRangeAndType('Value', octet, p.OCTET_MIN, p.OCTET_MAX))
}
...
export const decIPtoBinIP = (decOctets: iIPv4): string => {
  return toBinOctet(decOctets.o1) + toBinOctet(decOctets.o2) + toBinOctet(decOctets.o3) + toBinOctet(decOctets.o4)
}
